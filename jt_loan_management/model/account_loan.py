...
class AccountLoan(models.Model):
    ...
    other_services=fields.Char('Other Services')
    ...
    total_received_interest_balance = fields.Float('Interest Recieved', compute='cal_rec_princi_inter')
    total_received_principal_balance = fields.Float('Principal Recieved', compute='cal_rec_princi_inter')
    first_payment_due = fields.Date(string="Installment Due Date", copy=False)
    loan_payment_change_ids = fields.One2many('loan.changes',inverse_name='loan_id', string="Loan Changes", readonly=True)
    ...
    penalty_pending = fields.Float('Penalty Pending', compute='compute_penalty_pending', store=True, readonly=True)
    penalty_received = fields.Float('Penalty Received', compute='compute_penalty_pending', store=True, readonly=True)
